...
static void handle_arrow_click(WORD win_id,WORD object,WORD msg) {
	...
	Srv_wind_update(EVHD_APID,BEG_MCTRL);
	...
	Srv_wind_update(EVHD_APID,END_MCTRL);
}
...
static void handle_slider_click(WORD win_id, WORD elem,WORD owner,OBJECT *tree) {
    WORD      last_x = mouse_x,
              last_y = mouse_y,
              new_x = last_x,
              new_y = last_y,
	  ...
	  if((mouse_button & LEFT_BUTTON & changed)) {
    	...
    	if(status & WIN_DESKTOP) {
          ...
          COMMSG	m;
            ...
            m.length = 0;
            m.msg0 = win_id;
            Srv_wind_get(win_id,WF_UNICONIFY,&m.msg1,&m.msg2,&m.msg3,&m.msg4);
  	...
  	if((globals.mouse_owner == -1) ||
	    ...
	    if((*rl)->event.flag1 != -1) {
			...
			if(sendflag) {
				...
				Srv_put_event(tmp->event.apid,&m,sizeof(EVNTREC) + sizeof(EVNTREC_MOUSE));
		    	...
		    	if(changebut & LEFT_BUTTON & (!mouse_button)) {
						...
						if(nmenu == globals.pmenutad) {
							...
							if(mr) {
								...
								if(skeys & K_CTRL) {
									Srv_appl_control(mr->ai->id,APC_KILL);
								}
								else {
									COMMSG m;
									m.sid = 0;
