  ...
  if (text) {		
    ...
    case G_FBOXTEXT:
    {
      ...
      case SMALL:			/* Use the small system font (probably 8 point) */
        Vdi_vst_font(vid,globals->common->fnt_small_id);
        Vdi_vst_point(vid,globals->common->fnt_small_sz,&temp,&temp,&temp,&temp);
...
Objc_init_module (WORD apid) {	
  ...
  Vdi_vq_extnd (globals->common->vid,1,work_out);
  
  num_planes = work_out[4];
  
  Vdi_v_show_c (globals->common->vid,0);
